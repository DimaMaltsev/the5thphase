...
var world = (function(){
	...
	function addMovingObject( params ){
		// TOADD : obj validate check
		if( params == undefined ) return;
		var obj = new object( params )
	...
	function drawBox( x , y , val ){
		...
		val && ( context.fillStyle	=	"rgba("+ Math.floor( Math.random() * 255 ) + "," + Math.floor( Math.random() * 255 ) + "," + Math.floor( Math.random() * 255 ) + ", 0.7)")
    val && context.fill();
		context.lineWidth = 1;
	...
	function translateCoord( x , y ){
		var s = config.scale
		x = screen.width / 2 + x - ig.game.screen.x
		y = screen.height/ 2 + y - ig.game.screen.y
		return { x : x , y : y }
	}
	...
	return {
		...
		,	test : function(){ return movingObjects[0];/*physics.step( matrix , movingObjects , bsize );*/ }
